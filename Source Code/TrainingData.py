...
path = os.path.dirname('C:\\Users\\RAD5\\Desktop\\Student Resources New\\MSTAR\\Training Data')
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        if i % int(datacode.train.num_examples/batch_size) == 0: 
            val_loss = session.run(cost, feed_dict=feed_dict_val)
            epoch = int(i / int(datacode.train.num_examples/batch_size))    
            
            show_progress(epoch, feed_dict_tr, feed_dict_val, val_loss)
            saver.save(session, 'C:\\Users\\RAD5\\Desktop\\Student Resources New\\MSTAR\\mstar-model') 
